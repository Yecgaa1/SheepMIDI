...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_Start
********************************************************************************
*
* Summary:
...
void Clk_Tmr_Button_Start(void) 
{
    ...
    Clk_Tmr_Button_CLKEN |= Clk_Tmr_Button_CLKEN_MASK;
	Clk_Tmr_Button_CLKSTBY |= Clk_Tmr_Button_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clk_Tmr_Button_Stop
********************************************************************************
*
* Summary:
...
void Clk_Tmr_Button_Stop(void) 
{
    ...
    Clk_Tmr_Button_CLKEN &= (uint8)(~Clk_Tmr_Button_CLKEN_MASK);
	Clk_Tmr_Button_CLKSTBY &= (uint8)(~Clk_Tmr_Button_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_Tmr_Button_CLKEN & Clk_Tmr_Button_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_Tmr_Button_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_Tmr_Button_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_Tmr_Button__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_Tmr_Button_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_Tmr_Button_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_Tmr_Button_GetSourceRegister();
    uint16 oldDivider = Clk_Tmr_Button_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_Tmr_Button_CLKEN & Clk_Tmr_Button_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clk_Tmr_Button_DIV_PTR, clkDivider);
                Clk_Tmr_Button_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clk_Tmr_Button_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clk_Tmr_Button_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_Tmr_Button_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clk_Tmr_Button_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_Tmr_Button_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_Tmr_Button_MOD_SRC |= modeBitMask & (uint8)Clk_Tmr_Button_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_Tmr_Button_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_Tmr_Button_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_Tmr_Button_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_Tmr_Button_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_Tmr_Button_MOD_SRC & (uint8)(Clk_Tmr_Button_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_Tmr_Button_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_Tmr_Button_GetDividerRegister();
    uint8 oldSrc = Clk_Tmr_Button_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_Tmr_Button_MOD_SRC |= CYCLK_SSS;
        Clk_Tmr_Button_MOD_SRC =
            (Clk_Tmr_Button_MOD_SRC & (uint8)(~Clk_Tmr_Button_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_Tmr_Button_MOD_SRC & Clk_Tmr_Button_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_Tmr_Button_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_Tmr_Button_PHASE = clkPhase & Clk_Tmr_Button_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_Tmr_Button_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_Tmr_Button_PHASE & Clk_Tmr_Button_PHASE_MASK;
}
