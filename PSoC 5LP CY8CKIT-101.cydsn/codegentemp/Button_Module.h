...
********************************************************************************
 ...
 【注】：
  1. 对外API函数为：
       Button_Module_Start()、Button_Module_GetPBStatus()
       Button_Module_SetEquivSglclickGen()、Button_Module_GetEquivSglclickGen()
       Button_Module_GetMessage()、Button_Module_SendMessage()
       应用程序收发消息部分的代码需兼容无OS和FreeRTOS时，可使用：
       _Button_Module_GetMessage()、_Button_Module_SendMessage()
  2. 支持两种按键单击消息生成方式：
  ...
  6. 移植本按键模块到不同软硬件平台上时，仅需考虑本文件和Button_Module.c中带!!P标记
     的注释处的内容指引。
  7. 应用程序可配置的功能和参数位于本文件中带!A标记的注释处。
********************************************************************************
*/
#ifndef __Button_Module_H_
#define __Button_Module_H_
...
/*!!P 选择按键模块是否使用某操作系统的消息队列服务，NON_OS时使用自定义的按键消息队列，将根据所选生成代码。
      - Button_Module.c中：
        按键消息循环队列所需变量、宏的定义
        发送消息的内联版或OS版宏函数定义（Button_Module_Handler()函数中使用）
        Button_Module_Start()中的按键消息循环队列初始化
        自定义的按键消息循环队列的消息收发函数定义和实现（仅NON_OS时）
      - Button_Module.h中：
        Button_Module_Start()移植实现所需的OS队列头文件包含（仅FreeRTOS时）
        收发消息的宏函数定义、API函数声明（仅NON_OS时）
        收发消息宏函数定义（仅FreeRTOS时）
      OTHER_OS时这些代码需用户添加
*/
/* 0==宏NON_OS, 1==宏FREERTOS, 2==宏OTHER_OS */
#define PORTING_TO_OS_Q           0
...
*/
#define NUMPOW_BUTTONS_PER_BANK   -1
...
*/
#define SGL_CLICK_MSG_GEN_METHOD  1

/*!A 按键长按动作检测配置字（每位对应一个按键(b0位对应0号按键)，位值1表示需检测长按）*/
#define LONG_PRESS_DET_CONFIG     3ul
/*!A 按键双击动作检测配置字（每位对应一个按键(b0位对应0号按键)，位值1表示需检测双击）*/
#define DBL_CLICK_DET_CONFIG      0ul
...
#define LONG_PRESS_FR_DBLCLICK_SUPPORT  0

/*!A 设置是否在长按期间，生成等效的连续单击消息（每位对应一个按键(b0位对应0号按键)，位值1表示需生成）*/
#define EQUIV_SGLCLICK_GEN_CONFIG     3ul
...
#define BUTTON_EQUIV_SGLCLICK_INTERVAL_MS  100ul
 ...
 **************************************************************************/
/* `#START Button_Module_BUTTON_XXX_MS_Redefs` */
#undef BUTTON_LONGPRESS_HOLDTIME_MS
#define BUTTON_LONGPRESS_HOLDTIME_MS  1000ul
/* `#END` */
...
#define BUTTON_ON                 1ul  /* 单个按键闭合时的“逻辑”值（不一定需为引脚电平值）*/
#if BUTTON_ON == 0
...
*/
#include <CyLib.h>
#define ENT_CRITICAL_SEC          CyEnterCriticalSection
#define EXT_CRITICAL_SEC          CyExitCriticalSection
...
【注意】：Button_Module_Start()应在Button_Module.c中定义实现
          另外，同在Button_Module.c中的内部函数Button_Module_GetCurrentPBStatus()，也与移植相关，也需定义实现。
*/
/* 移植实现所需的I/O头文件包含 */
#include <cypins.h>
...
#define Button_Module_GetVersion()            ((uint16)_BUTTON_MODULE_VER_)

/* 按键消息队列不使用操作系统的消息队列服务时，收发消息的宏函数定义、API函数声明 */
#if PORTING_TO_OS_Q == NON_OS
  /* 可使用这两个简洁形式的宏函数代替紧接下方的对应版本 */
  #define Button_Module_GetMessage()               _Button_Module_GetMessage((uint8 *)0)
  #define Button_Module_SendMessage(button_msg)    _Button_Module_SendMessage(button_msg, (uint8 *)0)
...
/*!!P 按键消息队列使用操作系统的消息队列服务时，收发消息宏函数定义 */
  ...
  #define Button_Module_GetMessage(msg_pvar)        ((xQueueReceive(qbutton, msg_pvar, 0) == \
                                                     errQUEUE_EMPTY)? 0xFFu : *(msg_pvar))
  #define Button_Module_SendMessage(msg, msg_pvar)  (*(msg_pvar) = (msg),                    \
                                                     ((xQueueSend(qbutton, msg_pvar, 0) ==pdPASS)? 1u :0))
  /* 别名，兼容不使用OS的消息队列服务时的版本 */
  #define _Button_Module_GetMessage           Button_Module_GetMessage
  #define _Button_Module_SendMessage          Button_Module_SendMessage
#else
  /* 对于其它OS，请于下方注释#START ～ #END之间添加收发消息宏函数的定义 */
  /*************************************************************************
   *  Place your queue declarations and Rx/Tx macros under other OS
   ************************************************************************/
  /* `#START Button_Module_Queue_OS_Decs_RTxMacros` */
