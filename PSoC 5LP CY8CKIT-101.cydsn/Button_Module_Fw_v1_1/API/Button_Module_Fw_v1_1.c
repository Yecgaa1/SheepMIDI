 ...
 **************************************************************************/
/* `#START `$INSTANCE_NAME`_Add_Header_Code` */
...
/*!!P 按键消息循环队列所需变量、宏的定义 */
  ...
  /* `$INSTANCE_NAME`_Handler()函数中，QBUTTON_SENDMESSAGExxx()宏函数用名为_button_event的变量(保存待发消息)发送消息 */
  ...
  /*
    ...
    // `$INSTANCE_NAME`_Handler()函数中，QBUTTON_SENDMESSAGExxx()宏函数用名为_button_event的数组元素msg_idx发送消息
  ...
  */
   ...
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_Queue_OS_Defs` */
...
/*!!P `$INSTANCE_NAME`_Handler()函数中所用的，发送消息的内联版或OS版宏函数定义 */
#if PORTING_TO_OS_Q == NON_OS
  /* _`$INSTANCE_NAME`_SendMessage()函数的内联版，缩短耗时，仅于本文件中可见 */
  ...
  */
   ...
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_SendMsgMacro_OS_Defs` */
...
static
#if (PORTING_TO_DVK == CY8CKIT_062) || (PORTING_TO_DVK == CY8CKIT_050)|| \
    (PORTING_TO_DVK == CY8CKIT_050B_REVC) || (PORTING_TO_DVK == CY8CKIT_046)
  inline
#endif /* PORTING_TO_DVK */
    ButtonBank `$INSTANCE_NAME`_GetCurrentPBStatus(uint8 bank);


/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_Start
********************************************************************************
*
*  【功    能】
*    使用GPIO固件初始化按键模块，含初始化按键消息循环队列
*
*  【入口参数】
*    无
*
...
uint8 `$INSTANCE_NAME`_Start(void)
{
  ...
  /* 按键相关I/O初始化 */
#if PORTING_TO_DVK == CY8CKIT_062
  /* 将P0.4、P12.5均配置为：GPIO模式、电阻上拉、输出高、禁止引脚中断 */
  Cy_GPIO_Pin_Init(GPIO_PRT0, 4, &pinConfig);
  Cy_GPIO_Pin_Init(GPIO_PRT12, 5, &pinConfig);
#elif (PORTING_TO_DVK == CY8CKIT_050) || (PORTING_TO_DVK == CY8CKIT_030)
  /* 将P6[1]、P15[5]均配置为：电阻上拉、输出高 */
  CyPins_SetPin(CYREG_PRT6_PC1);
  CyPins_SetPin(CYREG_IO_PC_PRT15_PC5);
  CyPins_SetPinDriveMode(CYREG_PRT6_PC1, CY_PINS_DM_RES_UP);
  CyPins_SetPinDriveMode(CYREG_IO_PC_PRT15_PC5, CY_PINS_DM_RES_UP);
#elif PORTING_TO_DVK == CY8CKIT_050B_REVC
  /* 将P6[1]、P15[5]分别配置为：高阻数字、电阻上拉且输出高 */
  CyPins_SetPinDriveMode(CYREG_PRT6_PC1, CY_PINS_DM_DIG_HIZ);
  CyPins_SetPin(CYREG_IO_PC_PRT15_PC5);
  CyPins_SetPinDriveMode(CYREG_IO_PC_PRT15_PC5, CY_PINS_DM_RES_UP);
#elif PORTING_TO_DVK == CY8CKIT_046
  /* 将P0[7]配置为：电阻上拉、输出高 */
  CY_SYS_PINS_SET_PIN(CYREG_PRT0_DR, 7);
  CY_SYS_PINS_SET_DRIVE_MODE(CYREG_PRT0_PC, 7, CY_SYS_PINS_DM_RES_UP);
#endif /* PORTING_TO_DVK */
  /* 对于其它开发套件或更多按键时，请于下方注释#START ～ #END之间添加按键相关I/O初始化代码 */
  /*************************************************************************
   *  Place your additional button pin initializations
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_Add_Button_Pin_Inits` */
  ...
  */
  ...
  if(qbutton == NULL)
  {
     ...
     **********************************************************************/
    /* `#START `$INSTANCE_NAME`_Queue_OS_Init` */
  ...
  else
  {
     ...
     **********************************************************************/
    /* `#START `$INSTANCE_NAME`_Queue_OS_Reset` */
...
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_GetCurrentPBStatus
********************************************************************************
*
*  【功    能】
...
#if PORTING_TO_DVK == CY8CKIT_062
  /* P0.4(SW2), P12.5(SW8) */
  val |= (ButtonBank)((Cy_GPIO_Read(GPIO_PRT0, 4) ==0)? BUTTON_ON : BUTTON_OFF);
  val |= (ButtonBank)((Cy_GPIO_Read(GPIO_PRT12, 5) ==0)? BUTTON_ON : BUTTON_OFF) <<1;
#elif (PORTING_TO_DVK == CY8CKIT_050) || (PORTING_TO_DVK == CY8CKIT_050B_REVC) || \
      (PORTING_TO_DVK == CY8CKIT_030)
  /* P6[1](SW2), P15[5](SW3) */
  val |= (ButtonBank)((CyPins_ReadPin(CYREG_PRT6_PC1) ==0)? BUTTON_ON : BUTTON_OFF);
  val |= (ButtonBank)((CyPins_ReadPin(CYREG_IO_PC_PRT15_PC5) ==0)? BUTTON_ON : BUTTON_OFF) <<1;
#elif PORTING_TO_DVK == CY8CKIT_046
  /* P0[7](SW2) */
  val |= (ButtonBank)((CY_SYS_PINS_READ_PIN(CYREG_PRT0_PS, 7) ==0)? BUTTON_ON : BUTTON_OFF);
#endif /* PORTING_TO_DVK */
  /*************************************************************************
   *  Place your additional button status reads
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_Add_Button_Status_Reads` */
...
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_GetPBStatus
********************************************************************************
*
*  【功    能】
...
*             数`$INSTANCE_NAME`_GetPBStatus()
*          6. 可配置各按键于其长按期间生成等效的连续单击消息
*             宏BUTTON_EQUIV_SGLCLICK_INTERVAL_MS设置等效单击消息的发送间隔，宏值
*             为0时全局禁止此功能。
*             `$INSTANCE_NAME`_SetEquivSglclickGen()和`$INSTANCE_NAME`_GetEquivSglclickGen()
*             函数分别设置和获取某按键的配置。
*
*  【入口参数】
*    bank：按键不分页时，该参数值不使用
...
void `$INSTANCE_NAME`_Handler(uint8 bank)
{
/* 以下最外层#if...#endif为：有需要检测扩展动作的按键时的变量定义 */
  ...
  /* 读各按键当前开合状态 */
  button_status_cur = `$INSTANCE_NAME`_GetCurrentPBStatus(bank);
    ...
    if(((act_flags >> iButton) & (ButtonBank)1u) != 0)  /* 第iButton号按键有动作 */
    {
      ...
      else                                          /* 第iButton号按键松开 */
      {
        ...
        { /* 不需检测该按键的长按和双击时，判定为松开，发送iButton号按键松开消息
           注：此为按键动作检测配置字不合理的防护性处理，若能遵`$INSTANCE_NAME`.h所述配置，删除此else if */
          UPDATE_ACTION_TYPE(button_action_type[iButton >>2], idx_bit, BUTTON_ACTION_TYPE_RELEASINGUP);
          QBUTTON_SENDMESSAGE_EX(iButton, BUTTON_ACTION_TYPE_RELEASINGUP, _button_event PTR_OFFSETS, idx_bit);
        }
...
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_SetEquivSglclickGen
********************************************************************************
*
*  【功    能】
...
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_GetEquivSglclickGen
********************************************************************************
*
*  【功    能】
...
/*******************************************************************************
*  函数名：_`$INSTANCE_NAME`_GetMessage
********************************************************************************
*
*  【功    能】
*    从队首获取一条消息
...
/*******************************************************************************
*  函数名：_`$INSTANCE_NAME`_SendMessage
********************************************************************************
*
*  【功    能】
*    向队尾添加一条消息
