...
********************************************************************************
 ...
 【注】：
  1. 对外API函数为：
       `$INSTANCE_NAME`_Start()、`$INSTANCE_NAME`_Handler()、`$INSTANCE_NAME`_GetPBStatus()
...
/* 开发套件宏名及其对应编号 */
#define GENERIC_DVK               0
#define CY8CKIT_062               1
#define CY8CKIT_050               2
#define CY8CKIT_050B_REVC         3
#define CY8CKIT_046               4
#define CY8CKIT_030               5

/*!!P 选择移植至的开发套件，将根据所选，生成板上所配用户机械按键的相关代码，及其它的编译器相关代码。
      - `$INSTANCE_NAME`.c中：
        `$INSTANCE_NAME`_Start()的相关实现代码
        `$INSTANCE_NAME`_GetCurrentPBStatus()的相关实现代码
      - `$INSTANCE_NAME`.h中：
        uint8、uint16、uint32基本数据类型定义
        进入/退出临界区函数名的宏替换
        `$INSTANCE_NAME`_Start()移植实现所需的头文件包含
      GENERIC_DVK时这些代码需用户添加
*/
/* 0==宏GENERIC_DVK,       1==宏CY8CKIT_062, 2==宏CY8CKIT_050, 
   3==宏CY8CKIT_050B_REVC, 4==宏CY8CKIT_046, 5==宏CY8CKIT_030  */
#define PORTING_TO_DVK            `$PORTING_TO_DVK`
...
/*!!P 定义uint8、uint16、uint32基本数据类型 */
  ...
  #include <cy_syslib.h>
      ...
      (PORTING_TO_DVK == CY8CKIT_046) || (PORTING_TO_DVK == CY8CKIT_030)
  #include <cytypes.h>
#else
  #warning Please define the basic data types: uint8, uint16, uint32, according to the compiler used
  /*【注意】：对于其它开发套件，需根据所用编译器于下方注释#START ～ #END之间正确定义
              uint8、uint16、uint32数据类型。
     以下被注释掉的3行定义仅为使PSoC Creator自带编译器编译通过的示例，未必符合实情。
  */
  /* typedef unsigned char  uint8;
     typedef unsigned short uint16;
     typedef unsigned long  uint32; */
  /*************************************************************************
   *  Place your basic data type definitions
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_Basic_Data_Type_Defs` */
...
#elif (PORTING_TO_DVK == CY8CKIT_050) || (PORTING_TO_DVK == CY8CKIT_050B_REVC) || \
      (PORTING_TO_DVK == CY8CKIT_046)
  #include <CyLib.h>
  #define ENT_CRITICAL_SEC              CyEnterCriticalSection
  #define EXT_CRITICAL_SEC              CyExitCriticalSection
#else
  #define ENT_CRITICAL_SEC
  #define EXT_CRITICAL_SEC
  /* 对于其它开发套件，若需要按键模块内部实现对按键消息队列操作的临界代码保护，
     请于下方注释#START ～ #END之间：
      #undef ENT_CRITICAL_SEC
      #undef EXT_CRITICAL_SEC
     再重新定义 ENT_CRITICAL_SEC、EXT_CRITICAL_SEC
  */
  /*************************************************************************
   *  Place your critical function substitutions
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_Critical_Func_Subs` */
...
/* 移植实现所需的I/O头文件包含 */
  ...
  #include <cypins.h>
#else
  /* 对于其它开发套件，请于下方注释#START ～ #END之间添加按键模块初始化所需的I/O头文件包含 */
  /*************************************************************************
   *  Place your I/O related includes
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_IO_Related_Incs` */
...
/* 移植实现所需的OS消息队列头文件包含 */
  ...
  #include "queue.h"
#else
  /* 对于其它OS，请于下方注释#START ～ #END之间添加OS队列服务所需的头文件包含 */
  /*************************************************************************
   *  Place your queue related includes under other OS
   ************************************************************************/
  /* `#START `$INSTANCE_NAME`_Queue_OS_Related_Incs` */
