...
static void `$INSTANCE_NAME`_Handle(void);
...
CY_ISR(MyISR)
{
    ...
    `$INSTANCE_NAME`_Handle();
}
#elif(PORTING_TO_CY8CKIT == 2)            // PSoC 6处理器、GCC编译器时
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_SysInt_Tmr_MKbLED_handler
********************************************************************************
*
*  【功    能】
*    矩阵键盘/LED 定时器中断服务程序
*    每发生一次计数值上溢/下溢时，调用一次`$INSTANCE_NAME`_Handle()。
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_SysInt_Tmr_MKbLED_handler(void)
{
    /***************************************************************************
     *  Place your additional codes
     **************************************************************************/
    /* `#START `$INSTANCE_NAME`_SysInt_Tmr_MKbLED_handler_header` */

    /* `#END` */

    if((Cy_TCPWM_GetInterruptStatus(`$INSTANCE_NAME`_Timer_MKbLED_HW,   \
            `$INSTANCE_NAME`_Timer_MKbLED_CNT_NUM) & CY_TCPWM_INT_ON_TC) != 0)
    {
        Cy_TCPWM_ClearInterrupt(`$INSTANCE_NAME`_Timer_MKbLED_HW,       \
                                `$INSTANCE_NAME`_Timer_MKbLED_CNT_NUM, CY_TCPWM_INT_ON_TC);
        `$INSTANCE_NAME`_Handle();

        /***************************************************************************
         *  Place your additional codes
         **************************************************************************/
        /* `#START `$INSTANCE_NAME`_SysInt_Tmr_MKbLED_handler_TC` */
...
{
    EN_WRITE(0);
    EN_SETDRIVEMODE(`$INSTANCE_NAME`_EN_DM_STRONG);
    RA_WRITE(0xFF);
    RA_SETDRIVEMODE(`$INSTANCE_NAME`_RA_DM_STRONG);
    COL_WRITE(0xFF);
    COL_SETDRIVEMODE(`$INSTANCE_NAME`_COL_DM_RES_UP);

#if(MESSAGE_GEN == 1)
    Qkey_Init();
#endif
    `$INSTANCE_NAME`_isr_MKbLED_StartEx(MyISR);
    iRow = 8;
    initvar = 0;
}
#elif(PORTING_TO_CY8CKIT == 2)            // PSoC 6处理器、GCC编译器时
...
void `$INSTANCE_NAME`_Start(void)
{
    ...
    /* 矩阵键盘/LED 定时器及其中断初始化 */
    const cy_stc_sysint_t `$INSTANCE_NAME`_SysInt_Tmr_MKbLED_cfg = {
        .intrSrc = (IRQn_Type)`$INSTANCE_NAME`_SysInt_Tmr_MKbLED__INTC_NUMBER,
        .intrPriority = `$INSTANCE_NAME`_SysInt_Tmr_MKbLED__INTC_CORTEXM4_PRIORITY
    };
    
    Cy_SysInt_Init(&`$INSTANCE_NAME`_SysInt_Tmr_MKbLED_cfg, `$INSTANCE_NAME`_SysInt_Tmr_MKbLED_handler);
    NVIC_ClearPendingIRQ(`$INSTANCE_NAME`_SysInt_Tmr_MKbLED_cfg.intrSrc);
    NVIC_EnableIRQ(`$INSTANCE_NAME`_SysInt_Tmr_MKbLED_cfg.intrSrc);

    `$INSTANCE_NAME`_Timer_MKbLED_Start();
    
#if(SCANS_PER_ROUND == 10)
    Cy_TCPWM_Counter_SetPeriod(`$INSTANCE_NAME`_Timer_MKbLED_HW,      \
                               `$INSTANCE_NAME`_Timer_MKbLED_CNT_NUM, `$CLOCK_FREQ_HZ`/500ul -1ul);
#elif(SCANS_PER_ROUND == 6)
    Cy_TCPWM_Counter_SetPeriod(`$INSTANCE_NAME`_Timer_MKbLED_HW,      \
                               `$INSTANCE_NAME`_Timer_MKbLED_CNT_NUM, `$CLOCK_FREQ_HZ`/303ul -1ul);
#endif
...
*    static uint8 rotary_action     标记旋转编码开关旋转动作，与函数`$INSTANCE_NAME`_GetKey()共享
*    static uint8 iRow;             标记将或正在扫描的行号，与函数`$INSTANCE_NAME`_Start()共享
*                                   `$INSTANCE_NAME`_Start()设置其初值为8
*    static uint8 initvar;          用于获得CHB, CHA按键初始状态，避免启动时产生此按键消息
*                                   与`$INSTANCE_NAME`_Start()共享，`$INSTANCE_NAME`_Start()设其初值为0
...
static void `$INSTANCE_NAME`_Handle(void)
{
    ...
    switch(iRow)
    {
        ...
        case 9:
            ...
            RA_SETDRIVEMODE(`$INSTANCE_NAME`_RA_DM_DIG_HIZ);
            COL_SETDRIVEMODE(`$INSTANCE_NAME`_COL_DM_OD_LO);
            COL_WRITE(0xFF);
            iRow = 10;
            break;
        ...
        case 10:
            ...
            EN_SETDRIVEMODE(`$INSTANCE_NAME`_EN_DM_DIG_HIZ);    // 读Down按键状态
            ...
            COL_SETDRIVEMODE(`$INSTANCE_NAME`_COL_DM_STRONG);    //! 加速驱动至高电平（相比电阻上拉时），
                                                            // 否则可能不能连续扫描所有按键行
...
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_SetBarLEDLevel
********************************************************************************
*
*  【功    能】
*    设置LED灯条的指示级
...
/*******************************************************************************
*  函数名：`$INSTANCE_NAME`_SimKeyAction
********************************************************************************
*
*  【功    能】
*    模拟按键动作时发送一条消息
...
{
#if(PORTING_TO_CY8CKIT == 1)              // PSoC 5LP处理器、GCC编译器时
    `$INSTANCE_NAME`_isr_MKbLED_Disable();
#elif(PORTING_TO_CY8CKIT == 2)            // PSoC 6处理器、GCC编译器时
    NVIC_DisableIRQ((IRQn_Type)`$INSTANCE_NAME`_SysInt_Tmr_MKbLED__INTC_NUMBER);
#endif
    if(onoff == KEYON)
    {
...
#if(PORTING_TO_CY8CKIT == 1)              // PSoC 5LP处理器、GCC编译器时
    `$INSTANCE_NAME`_isr_MKbLED_Enable();
#elif(PORTING_TO_CY8CKIT == 2)            // PSoC 6处理器、GCC编译器时
    NVIC_EnableIRQ((IRQn_Type)`$INSTANCE_NAME`_SysInt_Tmr_MKbLED__INTC_NUMBER);
#endif
}
