...
/*******************************************************************************
* Function Name: Clock_500Hz_Start
********************************************************************************
*
* Summary:
...
void Clock_500Hz_Start(void) 
{
    ...
    Clock_500Hz_CLKEN |= Clock_500Hz_CLKEN_MASK;
	Clock_500Hz_CLKSTBY |= Clock_500Hz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_500Hz_Stop
********************************************************************************
*
* Summary:
...
void Clock_500Hz_Stop(void) 
{
    ...
    Clock_500Hz_CLKEN &= (uint8)(~Clock_500Hz_CLKEN_MASK);
	Clock_500Hz_CLKSTBY &= (uint8)(~Clock_500Hz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_500Hz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_500Hz_CLKEN & Clock_500Hz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_500Hz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_500Hz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_500Hz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_500Hz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_500Hz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_500Hz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_500Hz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_500Hz_GetSourceRegister();
    uint16 oldDivider = Clock_500Hz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_500Hz_CLKEN & Clock_500Hz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_500Hz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_500Hz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_500Hz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_500Hz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_500Hz_MOD_SRC |= modeBitMask & (uint8)Clock_500Hz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_500Hz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_500Hz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_500Hz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_500Hz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_500Hz_MOD_SRC & (uint8)(Clock_500Hz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_500Hz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_500Hz_GetDividerRegister();
    uint8 oldSrc = Clock_500Hz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_500Hz_MOD_SRC |= CYCLK_SSS;
        Clock_500Hz_MOD_SRC =
            (Clock_500Hz_MOD_SRC & (uint8)(~Clock_500Hz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_500Hz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_500Hz_MOD_SRC & Clock_500Hz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_500Hz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_500Hz_PHASE = clkPhase & Clock_500Hz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_500Hz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_500Hz_PHASE & Clock_500Hz_PHASE_MASK;
}
