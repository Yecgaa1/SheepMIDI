...
#ifndef __MatrixKbLED_H_
#define __MatrixKbLED_H_

#define MESSAGE_GEN      1    // 生成按键消息

#define NUM_KEYROWS      4    // 此值限于R3~R0行按键区域，取值0~4
#define NUM_KEYCOLS      4    // 此值限于COL3~COL0列按键区域，取值0~4

#define NUM_LEDROWS      4    // 此值限于R7~R4行LED区域，取值0~4
#define NUM_LEDCOLS      4    // 此值限于COL3~COL0列LED区域，取值0~4

#define LVLNUM_BARLED   (NUM_LEDCOLS+4)  // LED条指示级别数（LED条灯数+1）
                                         // 指示级: 0(全灭) ~ LVLNUM_BARLED-1(全亮)
    
#define CUST_KEYSYM_TBL  0    // 使用用户自定义的按键行列号至按键符号查找表
#define SCANS_PER_ROUND  10   // 每轮所用的扫描次数，取值应为6（数值对应NUM_LEDROWS=4时）、
...
#define PORTING_TO_CY8CKIT     1     // 1——CY8CKIT-050、2——CY8CKIT-062

#if(PORTING_TO_CY8CKIT == 1)         // PSoC 5LP处理器、GCC编译器时
/* 以下头文件为PSoC 5LP处理器、GCC编译器实现宏函数时所需 */
#include "cydevice_trm.h"

#define MatrixKbLED_EN_DM_STRONG   6
#define MatrixKbLED_EN_DM_DIG_HIZ  1

#define MatrixKbLED_RA_DM_STRONG   6
#define MatrixKbLED_RA_DM_DIG_HIZ  1

#define MatrixKbLED_COL_DM_OD_LO   4
#define MatrixKbLED_COL_DM_RES_UP  2
#define MatrixKbLED_COL_DM_STRONG  6

/* 设置1位引脚EN的驱动模式，可选参数: MatrixKbLED_EN_DM_STRONG（强驱动）
                                      MatrixKbLED_EN_DM_DIG_HIZ（高阻数字输入） */
#define EN_SETDRIVEMODE(drvmode)   do{if((drvmode) == MatrixKbLED_EN_DM_STRONG){            \
...
/* 设置3位引脚RA的驱动模式，可选参数: MatrixKbLED_RA_DM_STRONG（强驱动）
                                      MatrixKbLED_RA_DM_DIG_HIZ（高阻数字输入） */
#define RA_SETDRIVEMODE(drvmode)   do{if((drvmode) == MatrixKbLED_RA_DM_STRONG){            \
...
/* 设置1~4位引脚COL的驱动模式，可选参数: MatrixKbLED_COL_DM_OD_LO（开漏无上拉、低驱动）
                                         MatrixKbLED_COL_DM_RES_UP（开漏有上拉、低驱动）
                                         MatrixKbLED_COL_DM_STRONG（强驱动）（SCANS_PER_ROUND=10时无需此模式）*/
#define COL_SETDRIVEMODE(drvmode)  do{if((drvmode) == MatrixKbLED_COL_DM_OD_LO){            \
                                        *(volatile uint8 *)CYREG_PRT12_DM2 |= 0x0F;         \
                                        *(volatile uint8 *)CYREG_PRT12_DM1 &= 0xF0;         \
                                        *(volatile uint8 *)CYREG_PRT12_DM0 &= 0xF0;}        \
                                      else if((drvmode) == MatrixKbLED_COL_DM_RES_UP){      \
...
#define MatrixKbLED_EN_DM_STRONG   CY_GPIO_DM_STRONG
#define MatrixKbLED_EN_DM_DIG_HIZ  CY_GPIO_DM_HIGHZ

#define MatrixKbLED_RA_DM_STRONG   CY_GPIO_DM_STRONG
#define MatrixKbLED_RA_DM_DIG_HIZ  CY_GPIO_DM_HIGHZ

#define MatrixKbLED_COL_DM_OD_LO   CY_GPIO_DM_OD_DRIVESLOW
#define MatrixKbLED_COL_DM_RES_UP  CY_GPIO_DM_PULLUP
#define MatrixKbLED_COL_DM_STRONG  CY_GPIO_DM_STRONG
