...
static inline ButtonBank `$INSTANCE_NAME`_GetCurrentPBStatus(uint8 bank);
static void `$INSTANCE_NAME`_Handler(uint8 bank);

/* 定时器中断服务函数 */
static void SysInt_Tmr_Button_handler(void);


/*******************************************************************************
*  函数名：SysInt_Tmr_Button_handler
********************************************************************************
*
*  【功    能】
*    按键模块定时器中断服务程序
*    每发生一次计数值上溢/下溢时，调用一次`$INSTANCE_NAME`_Handler()。
...
static void SysInt_Tmr_Button_handler(void)
{
   ...
   ************************************************************************/
  /* `#START SysInt_Tmr_Button_handler_header` */

  /* `#END` */

  if((Cy_TCPWM_GetInterruptStatus(Timer_Button_HW, Timer_Button_CNT_NUM) & CY_TCPWM_INT_ON_TC) != 0)
  {
    Cy_TCPWM_ClearInterrupt(Timer_Button_HW, Timer_Button_CNT_NUM, CY_TCPWM_INT_ON_TC);

#if NUMPOW_BUTTONS_PER_BANK >= 0
    `$INSTANCE_NAME`_Handler(bank);
    /* 按键分页时，切换为下一页号 */
    bank = (bank < (NUM_BUTTON_BANKS - 1u))? bank + 1u : 0;
#else
    `$INSTANCE_NAME`_Handler(0);
#endif /* NUMPOW_BUTTONS_PER_BANK */

    /***********************************************************************
     *  Place your additional codes
     **********************************************************************/
    /* `#START SysInt_Tmr_Button_handler_TC` */
...
{
  uint8 i;
  const cy_stc_sysint_t SysInt_Tmr_Button_cfg = {
                        .intrSrc = (IRQn_Type)SysInt_Tmr_Button__INTC_NUMBER,
                        .intrPriority = SysInt_Tmr_Button__INTC_CORTEXM4_PRIORITY
                       };
  ...
  */
  ...
  /* 定时器中断初始化 */
  Cy_SysInt_Init(&SysInt_Tmr_Button_cfg, SysInt_Tmr_Button_handler);
  NVIC_ClearPendingIRQ(SysInt_Tmr_Button_cfg.intrSrc);
  NVIC_EnableIRQ(SysInt_Tmr_Button_cfg.intrSrc);

  /* 定时器初始化 */
  Timer_Button_Start();
  Timer_Button_SetPeriod(BUTTON_SAMPLE_PERIOD_MS*1000ul);
...
static inline ButtonBank `$INSTANCE_NAME`_GetCurrentPBStatus(uint8 bank)
{
  ...
  /* 获取所有按键当前开合状态，或者第bank页按键当前状态。本例获取开发板上的所有用户机械按键状态。
     可在下方注释#START ～ #END之间添加获取更多按键的状态读取代码 */
  val  = (ButtonBank)(BUTTON_ALL_OFF << NUM_BUTTONS);
  val |= (ButtonBank)((Cy_GPIO_Read(Pin_SW2_PORT, Pin_SW2_NUM) ==0)? BUTTON_ON : BUTTON_OFF);
  val |= (ButtonBank)((Cy_GPIO_Read(Pin_SW8_PORT, Pin_SW8_NUM) ==0)? BUTTON_ON : BUTTON_OFF) <<1;
  /*************************************************************************
...
/* 以下最外层#if...#endif为：需要检测扩展动作的按键处理 ...... */
  ...
  /* 提前将本按键页各按键的上一状态，更新为当前状态。bank变量后续便可作其它用途 */
